...
def create_label_im(text, **kwargs):
    ...
    # Replace getsize() with getbbox() and calculate text size
...
def print_text():
    ...
    try:
        context = get_label_context(request)
        printer_index = int(request.params.get("printer", CONFIG["DEFAULT_PRINTER"]))
        printer_config = CONFIG["PRINTERS"][printer_index]
    except (LookupError, IndexError, ValueError) as e:
    ...
    qlr = BrotherQLRaster(printer_config["MODEL"])
    red = False
    ...
    if not DEBUG:
        try:
            be = BACKEND_CLASS(printer_config["PRINTER"])
            be.write(qlr.data)
            be.dispose()
            del be
...
def main():
    ...
    if args.printer:
        CONFIG["PRINTERS"][CONFIG["DEFAULT_PRINTER"]]["PRINTER"] = args.printer
    ...
    try:
        ...
        selected_backend = guess_backend(default_printer["PRINTER"])
    except ValueError:
        parser.error(
